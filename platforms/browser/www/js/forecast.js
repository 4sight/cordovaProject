...
$.getJSON('https://api.openweathermap.org/data/2.5/forecast?q=' + place + '&appid=cfc6d5b9b5c3a52a649857702433e58b',function(json){
	...
	var q;
	var rawTemperaturesTodayPlus5 = [];
	var r = 0;
	for (q = o + 1; r < 8; r++){
		console.log(q);
		rawTemperaturesTodayPlus5.push(JSON.stringify(json.list[(q)].main.temp));
		q++;
	}
	temperaturesTodayPlus5 = Array.from(rawTemperaturesTodayPlus5, x => (9 / 5 * (x - 273.15) + 32).toFixed(2));
	var highTodayPlus5 = Math.max(...temperaturesTodayPlus5);
	var lowTodayPlus5  = Math.min(...temperaturesTodayPlus5);
	document.getElementById('day5high').textContent = 'High: ' + highTodayPlus5;
	document.getElementById('day5low').textContent = 'Low: ' + lowTodayPlus5;
});